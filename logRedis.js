...
var Log = exports = module.exports = function Log(level, redis, redisKey) {
...
Log.prototype = {
    ...
    log: function (levelStr, args) {
        if (exports[levelStr] <= this.level) {
            var msg = fmt.apply(null, args).replace(/(\s*$)/g, "");
            var date = new Date();
            var time = date.toLocaleTimeString() + "." + date.getMilliseconds();
            var logMsg = '[' + time + ']' + ' ' + levelStr + ' ' + msg + '\n';
            this.redis.publish(this.redisKey, logMsg, function (err, result) {
                //to do
                if (err) {
                    console.log(err);
                }
